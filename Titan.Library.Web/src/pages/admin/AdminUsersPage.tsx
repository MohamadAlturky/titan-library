...
const emptyDraft: DraftFilters = { search: '', userType: 'all' };
const PAGE_SIZE = 10;
...
export function AdminUsersPage() {
    ...
    async (filters: AppliedFilters, currentPage: number, sort: SortingState) => {
      ...
      try {
        ...
        const res = await adminService.getUsers({
          ...
          sortBy: sortField ? (sortField.id as 'id' | 'name' | 'email' | 'createdAt' | 'isActive') : undefined,
  ...
  useEffect(() => {
    fetchUsers(applied, page, sorting);
  }, [applied, page, sorting, fetchUsers]);

  const handleApply = () => {
    setApplied({
      search: draft.search.trim() || undefined,
      userType: draft.userType === 'all' ? undefined : Number(draft.userType),
    });
    setPage(1);
  };
  ...
  const columns = [
    ...
    col.accessor('userType', {
      header: 'Role',
      cell: ({ getValue }) => {
        const v = userTypeLabel(getValue());
        return <Badge variant={v as 'customer' | 'author'}>{v.charAt(0).toUpperCase() + v.slice(1)}</Badge>;
      },
    }),
    col.accessor('isActive', {
      header: 'Status',
      cell: ({ getValue }) => (
        <Badge variant={getValue() ? 'active' : 'borrowed'}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Users" description="All registered customers and authors" />
            ...
            <div className="flex flex-col md:flex-row md:items-end gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="inline-flex p-1 bg-gray-200/50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl">
                  ...
                  ] as { value: UserTypeFilter; label: string }[]).map(opt => (
                    <button
                      key={opt.value}
                  ...
                  <tr>
                    <td colSpan={columns.length} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3 text-gray-400 dark:text-zinc-500">
                        ...
                        <p className="text-sm font-medium text-gray-600 dark:text-zinc-300">No users found</p>
                        {hasActiveFilters && (
